...
APP捉包把整个URL放进去TxStockAppUrl3，header转成JSON字符串之后放进去TxStockAppHeader3
微信微证券公众号捉包后header转成JSON字符串之后放进去TxStockWxHeader3
export TxStockAppUrl3='https://wzq.ten....#https://wzq.ten....#https://wzq.ten....'
export TxStockAppHeader3='{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}'
export TxStockWxHeader3='{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}'

重写食用
TxStockAppUrl3与TxStockAppHeader3：打开APP，点击头像->右上角金币->获取金币
TxStockWxHeader3：打开 腾讯自选股微信版|微证券 公众号，右下角好福利->福利中心
...
var TxStockAppUrl3
var TxStockAppHeader3
var TxStockWxHeader3
    ...
    if($request.url.indexOf("activity_task_daily.fcgi?") > -1 ||
       $request.url.indexOf("activity_task_continue.fcgi?") > -1) {
        if($request.url.indexOf("openid=") > -1)
        {
            //APP包
            let msgStr = ''
            $.setdata($request.url,'TxStockAppUrl3')
            $.log(`获取TxStockAppUrl3成功: ${$request.url}\n`)
            msgStr += `获取TxStockAppUrl3成功: ${$request.url}\n`
            $.setdata(JSON.stringify($request.headers),'TxStockAppHeader3')
            $.log(`获取TxStockAppHeader3成功: ${JSON.stringify($request.headers)}\n`)
            msgStr += `获取TxStockAppHeader3成功: ${JSON.stringify($request.headers)}\n`
        ...
        {
            //微信包
            $.setdata(JSON.stringify($request.headers),'TxStockWxHeader3')
            $.log(`获取TxStockWxHeader3成功: ${JSON.stringify($request.headers)}\n`)
            $.msg(`获取TxStockAppHeader3成功: ${JSON.stringify($request.headers)}\n`)
        }
    }
}
...
async function checkEnv()
{
    if($.isNode())
    {
        TxStockAppUrl3 = process.env.TxStockAppUrl3
        TxStockAppHeader3 = process.env.TxStockAppHeader3
        TxStockWxHeader3 = process.env.TxStockWxHeader3
    }
    else
    {
        TxStockAppUrl3 = $.getdata('TxStockAppUrl3')
        TxStockAppHeader3 = $.getdata('TxStockAppHeader3')
        TxStockWxHeader3 = $.getdata('TxStockWxHeader3')
    }
    
    if(!TxStockAppUrl3 || !TxStockAppHeader3 || !TxStockWxHeader3)
    {
        str1 = TxStockAppUrl3 ? "" : "TxStockAppUrl3"
        str2 = TxStockAppHeader3 ? "" : "TxStockAppHeader3"
        str3 = TxStockWxHeader3 ? "" : "TxStockWxHeader3"
        $.log(`未找到环境变量: ${str1} ${str2} ${str3}\n`)
        return false
    }
    
    if (TxStockAppUrl3.indexOf('#') > -1) {
        appUrlArrs = TxStockAppUrl3.split('#');
        console.log(`您选择的是用"#"隔开TxStockAppUrl3\n`)
    } else if (TxStockAppUrl3.indexOf('\n') > -1) {
        appUrlArrs = TxStockAppUrl3.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockAppUrl3\n`)
    } else {
        appUrlArrs = [TxStockAppUrl3]
    };
    ...
    if (TxStockAppHeader3.indexOf('#') > -1) {
        appHeaderArrs = TxStockAppHeader3.split('#');
        console.log(`您选择的是用"#"隔开TxStockAppHeader3\n`)
    } else if (TxStockAppHeader3.indexOf('\n') > -1) {
        appHeaderArrs = TxStockAppHeader3.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockAppHeader3\n`)
    } else {
        appHeaderArrs = [TxStockAppHeader3]
    };
    ...
    if (TxStockWxHeader3.indexOf('#') > -1) {
        wxHeaderArrs = TxStockWxHeader3.split('#');
        console.log(`您选择的是用"#"隔开TxStockWxHeader3\n`)
    } else if (TxStockWxHeader3.indexOf('\n') > -1) {
        wxHeaderArrs = TxStockWxHeader3.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockWxHeader3\n`)
    } else {
        wxHeaderArrs = [TxStockWxHeader3]
    };
