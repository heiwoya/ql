...
APP捉包把整个URL放进去TxStockAppUrl4，header转成JSON字符串之后放进去TxStockAppHeader4
微信微证券公众号捉包后header转成JSON字符串之后放进去TxStockWxHeader4
export TxStockAppUrl4='https://wzq.ten....#https://wzq.ten....#https://wzq.ten....'
export TxStockAppHeader4='{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}'
export TxStockWxHeader4='{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}'

重写食用
TxStockAppUrl4与TxStockAppHeader4：打开APP，点击头像->右上角金币->获取金币
TxStockWxHeader4：打开 腾讯自选股微信版|微证券 公众号，右下角好福利->福利中心
...
var TxStockAppUrl4
var TxStockAppHeader4
var TxStockWxHeader4
    ...
    if($request.url.indexOf("activity_task_daily.fcgi?") > -1 ||
       $request.url.indexOf("activity_task_continue.fcgi?") > -1) {
        if($request.url.indexOf("openid=") > -1)
        {
            //APP包
            let msgStr = ''
            $.setdata($request.url,'TxStockAppUrl4')
            $.log(`获取TxStockAppUrl4成功: ${$request.url}\n`)
            msgStr += `获取TxStockAppUrl4成功: ${$request.url}\n`
            $.setdata(JSON.stringify($request.headers),'TxStockAppHeader4')
            $.log(`获取TxStockAppHeader4成功: ${JSON.stringify($request.headers)}\n`)
            msgStr += `获取TxStockAppHeader4成功: ${JSON.stringify($request.headers)}\n`
        ...
        {
            //微信包
            $.setdata(JSON.stringify($request.headers),'TxStockWxHeader4')
            $.log(`获取TxStockWxHeader4成功: ${JSON.stringify($request.headers)}\n`)
            $.msg(`获取TxStockAppHeader4成功: ${JSON.stringify($request.headers)}\n`)
        }
    }
}
...
async function checkEnv()
{
    if($.isNode())
    {
        TxStockAppUrl4 = process.env.TxStockAppUrl4
        TxStockAppHeader4 = process.env.TxStockAppHeader4
        TxStockWxHeader4 = process.env.TxStockWxHeader4
    }
    else
    {
        TxStockAppUrl4 = $.getdata('TxStockAppUrl4')
        TxStockAppHeader4 = $.getdata('TxStockAppHeader4')
        TxStockWxHeader4 = $.getdata('TxStockWxHeader4')
    }
    
    if(!TxStockAppUrl4 || !TxStockAppHeader4 || !TxStockWxHeader4)
    {
        str1 = TxStockAppUrl4 ? "" : "TxStockAppUrl4"
        str2 = TxStockAppHeader4 ? "" : "TxStockAppHeader4"
        str3 = TxStockWxHeader4 ? "" : "TxStockWxHeader4"
        $.log(`未找到环境变量: ${str1} ${str2} ${str3}\n`)
        return false
    }
    
    if (TxStockAppUrl4.indexOf('#') > -1) {
        appUrlArrs = TxStockAppUrl4.split('#');
        console.log(`您选择的是用"#"隔开TxStockAppUrl4\n`)
    } else if (TxStockAppUrl4.indexOf('\n') > -1) {
        appUrlArrs = TxStockAppUrl4.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockAppUrl4\n`)
    } else {
        appUrlArrs = [TxStockAppUrl4]
    };
    ...
    if (TxStockAppHeader4.indexOf('#') > -1) {
        appHeaderArrs = TxStockAppHeader4.split('#');
        console.log(`您选择的是用"#"隔开TxStockAppHeader4\n`)
    } else if (TxStockAppHeader4.indexOf('\n') > -1) {
        appHeaderArrs = TxStockAppHeader4.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockAppHeader4\n`)
    } else {
        appHeaderArrs = [TxStockAppHeader4]
    };
    ...
    if (TxStockWxHeader4.indexOf('#') > -1) {
        wxHeaderArrs = TxStockWxHeader4.split('#');
        console.log(`您选择的是用"#"隔开TxStockWxHeader4\n`)
    } else if (TxStockWxHeader4.indexOf('\n') > -1) {
        wxHeaderArrs = TxStockWxHeader4.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockWxHeader4\n`)
    } else {
        wxHeaderArrs = [TxStockWxHeader4]
    };
