...
APP捉包把整个URL放进去TxStockAppUrl2，header转成JSON字符串之后放进去TxStockAppHeader2
微信微证券公众号捉包后header转成JSON字符串之后放进去TxStockWxHeader2
export TxStockAppUrl2='https://wzq.ten....#https://wzq.ten....#https://wzq.ten....'
export TxStockAppHeader2='{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}'
export TxStockWxHeader2='{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}'

重写食用
TxStockAppUrl2与TxStockAppHeader2：打开APP，点击头像->右上角金币->获取金币
TxStockWxHeader2：打开 腾讯自选股微信版|微证券 公众号，右下角好福利->福利中心
...
var TxStockAppUrl2
var TxStockAppHeader2
var TxStockWxHeader2
    ...
    if($request.url.indexOf("activity_task_daily.fcgi?") > -1 ||
       $request.url.indexOf("activity_task_continue.fcgi?") > -1) {
        if($request.url.indexOf("openid=") > -1)
        {
            //APP包
            let msgStr = ''
            $.setdata($request.url,'TxStockAppUrl2')
            $.log(`获取TxStockAppUrl2成功: ${$request.url}\n`)
            msgStr += `获取TxStockAppUrl2成功: ${$request.url}\n`
            $.setdata(JSON.stringify($request.headers),'TxStockAppHeader2')
            $.log(`获取TxStockAppHeader2成功: ${JSON.stringify($request.headers)}\n`)
            msgStr += `获取TxStockAppHeader2成功: ${JSON.stringify($request.headers)}\n`
        ...
        {
            //微信包
            $.setdata(JSON.stringify($request.headers),'TxStockWxHeader2')
            $.log(`获取TxStockWxHeader2成功: ${JSON.stringify($request.headers)}\n`)
            $.msg(`获取TxStockAppHeader2成功: ${JSON.stringify($request.headers)}\n`)
        }
    }
}
...
async function checkEnv()
{
    if($.isNode())
    {
        TxStockAppUrl2 = process.env.TxStockAppUrl2
        TxStockAppHeader2 = process.env.TxStockAppHeader2
        TxStockWxHeader2 = process.env.TxStockWxHeader2
    }
    else
    {
        TxStockAppUrl2 = $.getdata('TxStockAppUrl2')
        TxStockAppHeader2 = $.getdata('TxStockAppHeader2')
        TxStockWxHeader2 = $.getdata('TxStockWxHeader2')
    }
    
    if(!TxStockAppUrl2 || !TxStockAppHeader2 || !TxStockWxHeader2)
    {
        str1 = TxStockAppUrl2 ? "" : "TxStockAppUrl2"
        str2 = TxStockAppHeader2 ? "" : "TxStockAppHeader2"
        str3 = TxStockWxHeader2 ? "" : "TxStockWxHeader2"
        $.log(`未找到环境变量: ${str1} ${str2} ${str3}\n`)
        return false
    }
    
    if (TxStockAppUrl2.indexOf('#') > -1) {
        appUrlArrs = TxStockAppUrl2.split('#');
        console.log(`您选择的是用"#"隔开TxStockAppUrl2\n`)
    } else if (TxStockAppUrl2.indexOf('\n') > -1) {
        appUrlArrs = TxStockAppUrl2.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockAppUrl2\n`)
    } else {
        appUrlArrs = [TxStockAppUrl2]
    };
    ...
    if (TxStockAppHeader2.indexOf('#') > -1) {
        appHeaderArrs = TxStockAppHeader2.split('#');
        console.log(`您选择的是用"#"隔开TxStockAppHeader2\n`)
    } else if (TxStockAppHeader2.indexOf('\n') > -1) {
        appHeaderArrs = TxStockAppHeader2.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockAppHeader2\n`)
    } else {
        appHeaderArrs = [TxStockAppHeader2]
    };
    ...
    if (TxStockWxHeader2.indexOf('#') > -1) {
        wxHeaderArrs = TxStockWxHeader2.split('#');
        console.log(`您选择的是用"#"隔开TxStockWxHeader2\n`)
    } else if (TxStockWxHeader2.indexOf('\n') > -1) {
        wxHeaderArrs = TxStockWxHeader2.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockWxHeader2\n`)
    } else {
        wxHeaderArrs = [TxStockWxHeader2]
    };
